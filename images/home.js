...
GHomepage = {
	...
	BuildMainCapsuleItem: function( rgItem, strFeatureContext )
	{
		...
		else
		{
			...
			if ( strImageURL )
			{
				$ImgCtn.append( $J('<div/>', {'class': 'cluster_maincap_fill ' + (rgItemData.package_header ? 'package' : '') } )
					.append(
						$J('<img/>', {'class': 'cluster_maincap_fill_placeholder', src: 'http://store.akamai.steamstatic.com/public/images/v6/home/maincap_placeholder_616x353.gif' } ),
						$J('<img/>', {'class': 'cluster_capsule_image cluster_maincap_fill_bg', src: 'http://store.akamai.steamstatic.com/public/images/blank.gif', 'data-image-url': strImageURL } ),
						$J('<img/>', {'class': 'cluster_maincap_fill_header', src: 'http://store.akamai.steamstatic.com/public/images/blank.gif', 'data-image-url': strImageURL } )
					)
				);
			}
			else
		...
		else if( rgRecommendationReasons.recommended && GDynamicStore.s_rgRecommendedTags )
		{
			...
			if( rgMatchedTags.length == 0 )
				var $ReasonMain = $J('<div/>').addClass('main').addClass('bytags').html( "\u56e0\u60a8\u73a9\u7c7b\u4f3c\u6e38\u620f\u800c<strong>\u63a8\u8350<\/strong>" );
			else
				var $ReasonMain = $J('<div/>').addClass('main').addClass('bytags').html( "<strong>\u4e3a\u60a8\u63a8\u8350<\/strong>\uff0c\u4e0e\u60a8\u73a9\u8fc7\u7684\u6e38\u620f\u6709\u76f8\u4f3c\u6807\u7b7e" );
		...
		$J('.tab_item').each(function(i, j){
			...
			if( unAppId && GStoreItemData.rgAppData[ unAppId ] )
			{
				...
				if ( rgData['review_summary'] )
				{
					...
					$elReviewData.append( $J('<div>', {'class': 'title'}).text('总体用户评测：') );
				...
				if( rgData.tags )
				{
					...
					{
						var url = GStoreItemData.AddNavEventParamsToURL( 'http://store.steampowered.com/tag/zh-cn/TAGNAME/'.replace( /TAGNAME/, encodeURIComponent( rgData.tags[i] ) ), 'tab-preview' );
	...
	RenderRecommendedTags: function()
	{
		...
		for ( var i = 0; i < GDynamicStore.s_rgRecommendedTags.length; i++ )
		{
			...
			{
				var url = GStoreItemData.AddNavEventParamsToURL( 'http://store.steampowered.com/tag/zh-cn/TAGNAME/'.replace( /TAGNAME/, encodeURIComponent( rgTag.name ) ), 'gutter' );
	...
	RenderRecommendedForYouSpotlight: function()
	{
		...
		{
			var $Spotlight = GHomepage.RenderRecommendedSpotlight( rgRecommendedSpotlightOptions[0], '与您玩的相似的游戏' );
			if ( $Spotlight )
			{
		...
		if ( rgGamesShown.length < nGamesToShow )
		{
			...
			for ( var i = 0; i < rgWishlistItemsOnSale.length; i++ )
			{
				...
				var $Spotlight = GHomepage.RenderRecommendedSpotlight( rgWishlistItemsOnSale[i], '在您的愿望单中' );
				if ( $Spotlight )
				{
		...
		if ( rgGamesShown.length < nGamesToShow && GHomepage.rgFriendRecommendations )
		{
			for ( var i = 0; i < GHomepage.rgFriendRecommendations.length; i++ )
			{
				...
				{
					var $Spotlight = GHomepage.RenderRecommendedSpotlight( unAppID, '由好友推荐' );
					if ( $Spotlight )
					{
						$Element.append( $Spotlight );
		...
		{
			var $Spotlight = GHomepage.RenderRecommendedSpotlight( rgRecommendedSpotlightOptions[1], '与您玩的相似的游戏' );
			if ( $Spotlight )
			{
				$Element.append( $Spotlight );
...
CHomeSettings.prototype.DisplayPopup = function( $Btn )
{
	...
	if ( this.m_ApplicableSettings.new_on_steam )
		this.m_$Popup.append( this.RenderCheckbox( 'new_on_steam', 'Steam 新内容' ) );
	...
	if ( this.m_ApplicableSettings.recommended_by_curators )
		this.m_$Popup.append( this.RenderCheckbox( 'recommended_by_curators', '由您的 Steam 鉴赏家推荐' ) );
...
{
	if( sizeStr )
		return 'http://cdn.akamai.steamstatic.com/steam/' + 'apps/' + appid + '/' + filename.replace('.jpg', sizeStr + '.jpg');

	return 'http://cdn.akamai.steamstatic.com/steam/' + 'apps/' + appid + '/' + filename;
}
