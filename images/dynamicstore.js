...
GDynamicStore = {
	...
	Init: function( accountid, bForceRefresh, strOS )
	{
		...
		if ( accountid )
		{
			...
			var url = 'http://store.steampowered.com/dynamicstore/userdata/?id=' + accountid;
	...
	ModifyWishlist: function( appid, bRemove, fnOnSuccess, fnOnFail )
	{
		...
		if( bRemove )
			url = 'http://store.steampowered.com/api/removefromwishlist';
	...
	BuildBundleDescription: function( Bundle, $Description )
	{
		...
		{
			$Description.html( '<div>购买该捆绑包即可为所有 %2$s 件物品省 %1$s%%！</div>'.replace( '%1$s', Bundle.m_nDiscountPct ).replace( '%2$s', Bundle.m_cTotalItemsInBundle ).replace( '%%', '%' ) );
		}
	...
	PopulateRecommendedTagList: function()
	{
		...
		for( var i = 0; i < GDynamicStore.s_rgRecommendedTags.length && i < 5; i++ )
		{
			...
			var $Link = $J('<a/>', {'class': 'popup_menu_item', 'href': 'http://store.steampowered.com/tag/zh-cn/' + encodeURIComponent( tag.name ) });
			$Link.text( tag.name );
			$Element.append( $Link );
		}
	},
...
{
	$J.get(
		'http://store.steampowered.com/explore/howitworks/',
		{
			l : 'schinese'
		},
		function( data )
...
{
	return "http:\/\/cdn.akamai.steamstatic.com\/steamcommunity\/public\/images\/avatars\/" + strHash.substring( 0, 2 ) + '/' + strHash + strSize + '.jpg';
}
