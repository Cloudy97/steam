...
Cluster.BuildClusterCapsule = function( unAppID, unPackageID, strStatus, strFeature, nDepth, rgItemInfo )
{
	...
	{
		$CapCtn.append( $J('<img/>', {'class': 'cluster_capsule_image', src: 'http://store.akamai.steamstatic.com/public/images/v6/home/maincap_placeholder_616x353.gif', 'data-image-url': rgItemData.main_capsule } ) );
	}
	else
	{
		...
		$elWhyButton.click ( function ( event )
		{
			...
			} ).done ( function ( data )
			{
				...
				if ( data.tags )
				{
					...
					{
						strTooltipContent += '<a href="http://store.steampowered.com/tag/zh-cn/'+data.tags[ i ].name+'/">' + data.tags[ i ].name + '</a>';
					}
				}
